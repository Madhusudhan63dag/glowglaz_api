...
app.use(cors({
  origin: [
    'https://glowglazecommerce.vercel.app',
  ],
...
app.post("/send-order-confirmation", async (req, res) => {
  ...
  // Only send WhatsApp if enableWhatsApp is true
  if (enableWhatsApp) {
    /* ---------- build WhatsApp payload ---------- */
    ...
    try {
      ...
      whatsappError = waErr.response?.data || waErr.message;
  ...
  try {
    ...
    const response = {
      ...
      whatsappSent: enableWhatsApp ? whatsappSent : null,
      whatsappError: enableWhatsApp && !whatsappSent ? whatsappError : null
    };
